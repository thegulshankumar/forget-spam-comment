...
function forget_spam_comment_modify_action_url() {
	if (is_singular() && comments_open()) {
		...
		echo 'commentForm.action = "' . wp_make_link_relative(get_home_url()) . '/wp-comments-post.php?' . FORGET_SPAM_COMMENT_UNIQUE_KEY . '";';
		echo "};";
		echo "</script>\n";
	}
}
...
if ( $forget_spam_comment_post_request_received &&
     ...
     ! ( $forget_spam_comment_request_contain_key && strpos( $forget_spam_comment_referral_url, get_home_url() ) !== false ) ) {
